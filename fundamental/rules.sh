...
ip route add table 100 default dev eth1 via 192.168.22.1
ip route add table 101 default dev eth3 via 192.168.1.1
# select table based on mark on the packet
ip rule add fwmark 100 table 100
ip rule add fwmark 101 table 101
# for locally initiated connections
# ip rule add from 192.168.22.224 table 100
# ip rule add from 192.168.1.100 table 101
# other configs
echo 1 >/proc/sys/net/ipv4/ip_forward
echo 0 >/proc/sys/net/ipv4/conf/all/rp_filter

# eth1 — WAN connection 1, IP 192.168.22.224 default gw 192.168.22.1
# eth2 — WAN connection 2, IP 192.168.1.100 default gw 192.168.1.1
